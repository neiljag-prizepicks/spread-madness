...
**Tip lock:** `favorite_team_id` / `spread_from_favorite_perspective` are **not** applied on or after that game’s effective `scheduled_tip_utc` (overlay → bracket template). ESPN spread fetch uses the same rule. If no tip is known yet, imports still allow setting a line.
...
After **both teams** are known for a bracket game (including via feeder wins in `results.json`), ESPN’s **game summary** includes `pickcenter` lines (typically DraftKings on ESPN). [`fetch-espn-spreads.mjs`](fetch-espn-spreads.mjs) calls that endpoint per event and writes **`favorite_team_id`** and **`spread_from_favorite_perspective`** (negative = favorite laying points, same as your games template).
...
# Also fill scheduled_tip_utc from ESPN when overlay has no tip:
node scripts/fetch-espn-spreads.mjs --dates auto --include-tip
```
...
Spread writes use **current** wall time at each game check (`now < scheduled_tip_utc` in UTC ms), so a long run cannot keep updating lines after tip. `spread_updated_at` in the overlay is bumped **only** when the favorite or spread value changes.
